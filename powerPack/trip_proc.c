...
void trip_recording(int trip_code,double trip_data,char * st)
{
    ...
    TripInfoNow.CURRENT = rmsIu;
    TripInfoNow.VDC     = lpfVdc;
    TripInfoNow.HZ     = codeRateHz * reference_out;
...
{
	if(( protect_reg.bit.OVER_I_ADC)&&( abs(adcIu) > 3500)){
        trip_recording( ERR_OVER_CURRENT_U_PHASE, (float)(adcIu),"I adc over U ph");
		return ERR_OVER_CURRENT_U_PHASE;
	}
	if(( protect_reg.bit.OVER_I_ADC)&&( abs(adcIu) < 500)){
        trip_recording( ERR_OVER_CURRENT_U_PHASE, (float)(adcIu),"I adc under U ph");
		return ERR_OVER_CURRENT_U_PHASE;
	}
	if(( protect_reg.bit.OVER_I_ADC)&&( abs(adcIv) > 3500)){
        trip_recording( ERR_OVER_CURRENT_V_PHASE, (float)(adcIv),"I adc over V ph");
		return ERR_OVER_CURRENT_V_PHASE;
	}
	if(( protect_reg.bit.OVER_I_ADC)&&( abs(adcIv) < 500)){
        trip_recording( ERR_OVER_CURRENT_V_PHASE, (float)(adcIv),"I adc under V ph");
		return ERR_OVER_CURRENT_V_PHASE;
	}
	return 	0; 
}
