...
void SpaceVectorModulation( double *Vs_dq)
{
	...
	DutyCount[u] = (Uint16)(guiEpwmPeriodCount * DutyRatio[u] );
	DutyCount[v] = (Uint16)(guiEpwmPeriodCount * DutyRatio[v] );
	DutyCount[w] = (Uint16)(guiEpwmPeriodCount * DutyRatio[w] );
}  
...
{
	// 전류의 부호만을 사용하여 데드타임오차를 보상한다.

	T_d=1.0e-6*Dt_us;
	Dd=T_d*inv_Ts;	

//--- U phase		
	sgn_Is[as]=Is_abc[as]*inv_Min_Isw;							//전류의 크기에 따른 데드 타임계수  
...
//--- V phase	
	sgn_Is[bs]=Is_abc[bs]*inv_Min_Isw;							//전류의 크기에 따른 데드 타임계수  
...
//--- W phase	
	sgn_Is[cs]=Is_abc[cs]*inv_Min_Isw;							//전류의 크기에 따른 데드 타임계수  
