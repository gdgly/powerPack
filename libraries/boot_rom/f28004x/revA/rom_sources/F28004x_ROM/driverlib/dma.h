...
typedef enum
{
    ...
    DMA_TRIGGER_SPITXA     = 109,
    DMA_TRIGGER_SPIRXA     = 110,
    DMA_TRIGGER_SPITXB     = 111,
    DMA_TRIGGER_SPIRXB     = 112,
...
DMA_getTriggerFlagStatus(uint32_t base)
{
    ...
    if((HWREGH(base + DMA_O_CONTROL) & DMA_CONTROL_PERINTFLG) != 0U)
    {
        return(true);
    }
    else
...
//! the channel use DMA_haltChannel().
//!
//! \return None.
//
//*****************************************************************************
...
//!
//! \param base is the base address of the DMA channel control registers.
//! \param generateAtEnd is a flag to indicate the channel interrupt mode.
//!
//! This function sets the channel interrupt mode. When the \e generateAtEnd
//! parameter is \b true, the DMA channel interrupt will be generated at the
//! end of the transfer. If \b false, the interrupt will be generated at the
//! beginning of a new transfer. Generating at the beginning of a new transfer
...
//*****************************************************************************
static inline void
DMA_setInterruptMode(uint32_t base, bool generateAtEnd)
{
    // Check the arguments.
    ASSERT(DMA_isBaseValid(base));
...
//*****************************************************************************
extern void
DMA_configAddresses(uint32_t base, uint16_t *destAddr, uint16_t *srcAddr);
...
//! transferred word of a burst. These are 16-bit, signed values.
//!
...
//! The \e srcStep and \e destStep parameters specify the address step that
//! should be added to the source and destination addresses after each
//! transferred burst of a transfer. These are 16-bit, signed values. If a wrap
//! occurs, these step values will be ignored. Wrapping is configured with
//! DMA_configWrap().
//!
//! \note Note that regardless of what data size (configured by
//! DMA_configMode()) is used, parameters are in terms of 16-bits words.
...
//*****************************************************************************
extern void
DMA_configTransfer(uint32_t base, uint16_t transferSize, int16_t srcStep,
                   int16_t destStep);
...
//! occurs. These are 16-bit, signed values.
//!
//! \note Note that regardless of what data size (configured by
//! DMA_configMode()) is used, parameters are in terms of 16-bits words.
//!
...
//*****************************************************************************
extern void
DMA_configWrap(uint32_t base, uint16_t srcWrapSize, int16_t srcStep,
