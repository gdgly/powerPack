...
//*****************************************************************************
//
// Values that can be passed to the channel variable of
// HRPWM_setHRPWMSyncPulseSource() HRPWM_setHRMEPCtrlMode(),
// HRPWM_setHrCompLoadMode() function
//
//*****************************************************************************
typedef enum
{
    HRPWM_HRPWM_A = 0, //!< HrPWM A
    HRPWM_HRPWM_B = 8  //!< HrPWM B
}HRPWM_Channel;

//*****************************************************************************
//
// Values that can be passed to the mepEdgeMode variable of
// HRPWM_setHRMEPEdgeCtrlMode(),
//
//*****************************************************************************
typedef enum
{
    ...
    //! MEP controls rising edge of either CMPAHR and CMPBHR
    HRPWM_MEP_CTRL_CMP_RISING_EDGE = 1,
    //! MEP controls falling edge of either CMPAHR and CMPBHR
    HRPWM_MEP_CTRL_CMP_FALLING_EDGE = 2,
    //! MEP controls both edges of TBPHSHR  TBPRDHR
    HRPWM_MEP_CTRL_TBPHSHR_TBPRDHR_EDGE = 3
}HRPWM_MEPEdgeMode;

//*****************************************************************************
//
// Values that can be passed to the mepCtrlMode variable of
// HRPWM_setHRMEPCtrlMode()
//
//*****************************************************************************
typedef enum
{
    //! MEP controls the edge of CMPAHR/CMPBHR or TBPRDHR
    HRPWM_MEP_CTRL_DUTY_PERIOD = 0,
    //! MEP controls TBPHSHR
    HRPWM_MEP_CTRL_PHASE = 1
}HRPWM_MEPCtrlMode;

//*****************************************************************************
//
// Values that can be passed to HRPWM_setHRComparatorLoadMode,
// HRPWM_setHRRisingEdgeDelayLoadMode() and HRPWM_setHRFallingEdgeDelayLoadMode
// as the loadEvent parameter.
//
//*****************************************************************************
typedef enum
{
    //! load when counter equals zero
    HRPWM_HRPWM_LOAD_ON_CNTER_ZERO = 0,
    //! load when counter equals period
    HRPWM_HRPWM_LOAD_ON_CNTER_PERIOD = 1,
    //! load when counter equals zero or period
    HRPWM_HRPWM_LOAD_ON_CNTER_ZERO_PERIOD = 2
}HRPWM_HRPWMLoadMode;

//*****************************************************************************
//
// Values that can be passed to the outputOnB variable of
// HRPWM_setChannelBOutputPath()
//
//*****************************************************************************
typedef enum
{
    HRPWM_HRPWM_OUTPUT_ON_B_NORMAL = 0, //!< ePWMxB output is normal.
    HRPWM_HRPWM_OUTPUT_ON_B_INV_A  = 1  //!< ePWMxB output is inverted
                                        //!< version of ePWMxA signal
}HRPWM_ChannelBOutput;

//*****************************************************************************
//
// Values that can be passed to the syncPulseSource variable of
// HRPWM_setSyncPulseSource() function
//
//*****************************************************************************
typedef enum
{
...
//*****************************************************************************
//
// Values that can be passed to the HRPWM_disableCompLoad(),
// HRPWM_setCompCountValue (), HRPWM_setHrMepEdgeMode() function and
// HRPWM_compareLoad_config_t structure as compModule parameter.
//
//*****************************************************************************
typedef enum
{
...
//*****************************************************************************
//
// Values that can be passed to the mepDbEdge variable of
// HRPWM_setHRPWMDeadBandMEPEdgeCtrlMode()
//
//*****************************************************************************
typedef enum
{
...
#define HRPWM_enablePhaseShiftLoad               EPWM_enablePhaseShiftLoad
#define HRPWM_disablePhaseShiftLoad              EPWM_disablePhaseShiftLoad
...
#define HRPWM_setPhaseShift                      EPWM_setPhaseShift
...
#define HRPWM_setCounterCompareValue             EPWM_setCounterCompareValue
...
#define HRPWM_getTripZoneInterruptStatus        EPWM_getTripZoneInterruptStatus
#define HRPWM_getCycleByCycleTripZoneInterruptStatus \
                                   EPWM_getCycleByCycleTripZoneInterruptStatus
#define HRPWM_getOneShotTripZoneInterruptStatus    \
                                   EPWM_getOneShotTripZoneInterruptStatus
#define HRPWM_selectCycleByCycleTripZoneClearEvent  \
                                   EPWM_selectCycleByCycleTripZoneClearEvent
#define HRPWM_clearTripZoneInterruptFlag        EPWM_clearTripZoneInterruptFlag
#define HRPWM_clearCycleByCycleTripZoneInterruptFlag  \
                                   EPWM_clearCycleByCycleTripZoneInterruptFlag
#define HRPWM_clearOneShotTripZoneInterruptFlag   \
                                   EPWM_clearOneShotTripZoneInterruptFlag
...
#define HRPWM_enableValleyHardwareDelay       EPWM_enableValleyHardwareDelay
...
#define HRPWM_lockRegisters                   EPWM_lockRegisters
...
static inline void HRPWM_setTimeBasePeriod(uint32_t base, uint32_t periodCount)
{
    ...
    HWREGH(base + HRPWM_O_TBPRD) = periodCount << 8U;
}
...
//!   channel
//!    - HRPWM_HRPWM_A  - HrPWM A
//!    - HRPWM_HRPWM_B  - HrPWM B
//!  mepEdgeMode
//!    - HRPWM_MEP_CTRL_DISABLE              - HrPWM is disabled
//!    - HRPWM_MEP_CTRL_CMP_RISING_EDGE      - MEP (Micro Edge Positioner)
//!                                            controls rising edge CMPAHR
//!                                            or CMPBHR depending on the value
//!                                            channel.
//!    - HRPWM_MEP_CTRL_CMP_FALLING_EDGE     - MEP (Micro Edge Positioner)
//!                                            controls falling edge CMPAHR
//!                                            or CMPBHR depending on the value
//!                                            channel.
//!    - HRPWM_MEP_CTRL_TBPHSHR_TBPRDHR_EDGE - MEP (Micro Edge Positioner)
//!                                            controls both edges of
//!                                            TBPHSHR and TBPRDHR
//!
//! \return None.
//
...
//!    - HRPWM_HRPWM_B  - HrPWM B
//!   mepCtrlMode
//!    - HRPWM_MEP_CTRL_DUTY_PERIOD   - MEP (Micro Edge Positioner) controls
//!                                     the edge of CMPAHR/CMPBHR
//!                                     depending on the value of channel or
//!                                     TBPRDHR
//!    - HRPWM_MEP_CTRL_PHASE         - MEP (Micro Edge Positioner) controls
//!                                     TBPHSHR
//!
//! \return None.
//
...
//!    - HRPWM_HRPWM_B  - HrPWM B
//!  loadEvent
//!    - HRPWM_HRPWM_LOAD_ON_CNTER_ZERO       - load when counter equals zero
//!    - HRPWM_HRPWM_LOAD_ON_CNTER_PERIOD     - load when counter equals period
//!    - HRPWM_HRPWM_LOAD_ON_CNTER_ZERO_PERIOD  - load when counter equals zero
//!                                               or period
//!
//! \return None.
//
...
//! HRPWM_HRPWM_OUTPUT_ON_B_INV_A, ePWMxB output is an inverted version of
//! ePWMxA. If outputOnB is HRPWM_HRPWM_OUTPUT_ON_B_NORMAL, ePWMxB output is
//! ePWMxB.
//!
//! \return None.
//
...
//! scale HRMSTEP.
//! // TODO add  more description to the doxy comment
//! \return None.
//
//*****************************************************************************
                                   ...
                                   HRPWM_SyncPulseSource syncPulseSource)
{
	...
	if((uint16_t)syncPulseSource < HRPWM_PWMSYNC_SOURCE_COMPC_UP)
	{
		HWREGH(base + HRPWM_O_HRPCTL) =
          ((HWREGH(base + HRPWM_O_HRPCTL) & ~HRPWM_HRPCTL_PWMSYNCSELX_M) |
           ((uint16_t)syncPulseSource << 2U));
	}
	else
	{
        HWREGH(base + HRPWM_O_HRPCTL) =
                                         ...
                                         uint32_t compCount)
{
    ...
    if((uint16_t)compModule == HRPWM_COUNTER_COMPARE_A)
    {
        // write to CMPAHR
        HWREG(base + HRPWM_O_CMPA) = compCount << 8U;
    }
    else if((uint16_t)compModule == HRPWM_COUNTER_COMPARE_B)
    {
        // write to CMPBHR
...
//!
//! \b Note: redCount is a 23 bit value
//!
//! \return None.
//
//*****************************************************************************
static inline void HRPWM_setRisingEdgeDelay(uint32_t base, uint32_t redCount)
{
    ...
    HWREGH(base + HRPWM_O_DBREDHR) = redCount << 9U;
}
...
//!
//! This function sets the high resolution FED (Falling Edge Delay) count value.
//! The value of fedCount should be less than 128.
//!
//! \b Note: fedCount is a 23 bit value
//!
//! \return None.
//
//*****************************************************************************
static inline void HRPWM_setFallingEdgeDelay(uint32_t base,uint32_t fedCount)
{
    ...
    HWREGH(base + HRPWM_O_DBFEDHR) = fedCount << 9U;
}
