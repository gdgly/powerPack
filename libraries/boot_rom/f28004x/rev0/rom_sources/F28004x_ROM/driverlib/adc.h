...
//*****************************************************************************
//
//! Values that can be passed to ADC_registerInterrupt(),
//! ADC_unregisterInterrupt(), ADC_enableInterrupt(), ADC_disableInterrupt(),
...
//! function is configuring or accessing.
//
//*****************************************************************************
typedef enum
{
...
ADC_getInterruptStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    if((HWREGH(base + ADC_O_INTFLG) & (1U << (uint16_t)adcIntNum)) == 0U)
    {
        return(false);
    }
    else
...
ADC_enablePPBTwosComplement(uint32_t base, ADC_PPBNumber ppbNumber)
{
    ...
    //
    EALLOW;
    HWREGH(base + ppbOffset) |= ADC_PPB2CONFIG_TWOSCOMPEN;
    EDIS;
}
...
//*****************************************************************************
//
//! Sets the source SOC for an analog-to-digital converter interrupt.
//!
//! \param base is the base address of the ADC module.
//! \param adcIntNum is interrupt number within the ADC wrapper.
//! \param socNumber is the number of the start-of-conversion.
//!
//! This function sets which SOC is the source of an ADC interrupt.
//!
//! The \e socNumber number is a value \b ADC_SOC_NUMBERX where X is a number
//! from 0 to 15 specifying which SOC is to be configured on the ADC module
//! specified by \e base.
