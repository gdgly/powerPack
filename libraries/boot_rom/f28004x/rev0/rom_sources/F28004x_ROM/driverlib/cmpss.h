...
//! - \b CMPSS_INSRC_INTDAC - The internal DAC.
//! - \b CMPSS_INSRC_EXTPIN - An external pin.
//!
...
CMPSS_configLowComparator(uint32_t base, uint16_t config)
{
    ...
    HWREGH(base + CMPSS_O_COMPCTL) =
        (HWREGH(base + CMPSS_O_COMPCTL) & ~CMPSS_LOCMP_CTL_M) | (config << 8);
...
//! \b CMPSS_STS_HI_FILTOUT, \b CMPSS_STS_HI_LATCHFILTOUT,
//! \b CMPSS_STS_LO_FILTOUT, \b CMPSS_STS_LO_LATCHFILTOUT.
//
//*****************************************************************************
...
//! \param base is the base address of the comparator module.
//!
//! \note Value is read from the active register, not the shadow register to
...
CMPSS_configBlanking(uint32_t base, uint16_t pwmBlankSrc)
{
    ...
    //
    ASSERT(CMPSS_isBaseValid(base));
    ASSERT((pwmBlankSrc >= 1) && (pwmBlankSrc <= 16));
...
//*****************************************************************************
//
//! Enables a ePWM blanking signal to hold trip in reset.
//!
//! \param base is the base address of the comparator module.
//!
...
//*****************************************************************************
//
//! Disables a ePWM blanking signal from holding trip in reset.
//!
//! \param base is the base address of the comparator module.
//!
...
//! samples taken from the input that will monitored to determine when to
