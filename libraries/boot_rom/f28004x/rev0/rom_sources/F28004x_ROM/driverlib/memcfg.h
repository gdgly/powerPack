...
#define MEMCFG_XACCPROTX_M          (MEMCFG_GSXACCPROT0_FETCHPROT_GS0 |   \
...
//*****************************************************************************
//
// Values that can be passed to MemCfg_forceErrorStatus(),
// MemCfg_clearErrorStatus(), and MemCfg_getErrorAddress() as the
// stsFlag(s) parameter and returned by MemCfg_getErrorStatus().
//
...
//!
//! The \e ramSections parameter is logical OR of the following indicators:
...
//! value specified by the \e threshold parameter, the an interrupt is
...
//!
//! \return Returns the count of how many times a correctable error has
//! occurred.
//
//*****************************************************************************
static inline uint32_t
...
//! configuration of a RAM section. That means that calling
//! MemCfg_setProtection() or MemCfg_setLSRAMMasterSel() for a locked RAM section
//! will have no effect until MemCfg_unlockConfig() is called.
//!
//! The \e ramSections parameter is logical OR of one of the following sets of
//! indicators:
...
//! select configuration of a RAM section. That means that calling
//! MemCfg_setProtection() or MemCfg_setLSRAMMasterSel() for a locked RAM section
//! will have no effect. To lock the configuration in a nonpermanent way, use
//! MemCfg_lockConfig().
//!
//! The \e ramSections parameter is logical OR of one of the following sets of
//! indicators:
...
//*****************************************************************************
//
//! Starts initialization the specified RAM sections.
//!
...
//*****************************************************************************
//
//! Registers an interrupt handler for the RAM correctable error interrupt.
//!
//! \param handler is a pointer to the function to be called when the RAM
//! access violation interrupt occurs.
//!
//! This function registers the handler to be called when a RAM correctable
//! error interrupt occurs.  This function enables the global interrupt in the
//! interrupt controller; specific RAM access violation interrupts must be
//! enabled via MemCfg_enableCorrErrorInterrupt().  It is the interrupt handler's
